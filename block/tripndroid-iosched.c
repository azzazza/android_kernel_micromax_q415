...
SHOW_FUNCTION(tripndroid_writes_starved_show, td->writes_starved, 0);
...
STORE_FUNCTION(tripndroid_writes_starved_store, &td->writes_starved, INT_MIN, INT_MAX, 0);
...
static struct elv_fs_entry tripndroid_attrs[] = {
	TD_ATTR(sync_read_expire),
	TD_ATTR(sync_write_expire),
	TD_ATTR(async_read_expire),
	TD_ATTR(async_write_expire),
	TD_ATTR(writes_starved),
	TD_ATTR(fifo_batch),
	__ATTR_NULL
};
...
{
	return elv_register(&iosched_tripndroid);
}
